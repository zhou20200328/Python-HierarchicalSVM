...
def usage():
	print 'cmd --hier fname1 --telabel fname4 --predictionfolder fname5 --lossf fname --threshold v'
	
if __name__ == '__main__':
	import getopt, sys
	try:
		opts, args = getopt.getopt(sys.argv[1:], 'iot:h', ['help', 'hier=', 'telabel=', 'predictionfolder=', 'lossf=', 'threshold='])
	...
	if hier_fname == '' or test_label_fname == '' or prediction_output == '' or loss_fname == '' or threshold == -1:
		usage()
		sys.exit(1)
	...
	#make thresholds
	thresholds = [threshold for c in all_labels]
	...
	make_predicted_labels_prob_threshold(root, prediction_output, 'test_probs', predicted_labels, parents, thresholds)
	#output the loss
	...
	fd = open(prediction_output + '/predicted_labels_prob_threshold.txt', 'w')
	n = 0
