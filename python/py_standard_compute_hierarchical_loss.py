...
def usage():
	print 'cmd --hier fname1 --telabel fname4 --prlabel fname5 --lossf fname'
	
if __name__ == '__main__':
	import getopt, sys
	try:
		opts, args = getopt.getopt(sys.argv[1:], 'iot:h', ['help', 'hier=', 'telabel=', 'prlabel=', 'lossf='])
	...
	for opt, arg in opts:
		if opt in ('-h', '--help'):
			...
			predicted_label_fname = arg
		elif opt in ('--lossf'):
			loss_fname = arg
		
	if hier_fname == '' or test_label_fname == '' or predicted_label_fname == '' or loss_fname == '':
		usage()
		sys.exit(1)
	...
	predicted_labels = read_labels(predicted_label_fname)	
	#output the loss
