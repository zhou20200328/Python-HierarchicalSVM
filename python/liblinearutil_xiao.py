...
def train(arg1, arg2=None, arg3=None):
	...
	if param.cross_validation:
		...
		print 'end train'
		print 'start platt'
		liblinear.estimate_platt_models(prob, param, m)
		print 'end platt'
		m = toPyModel(m)		
...
def predict_label_score_prob(y, x, m, options=""):
	"""
	predict(y, x, m [, "options"]) -> (p_labels, p_acc, p_vals)
	...
	if predict_probability:
		...
		for xi in x:
			...
			prob = liblinear.sigmoid_predict(values[0], m.probA, m.probB)
			pred_labels += [label]			
			pred_values += values
			pred_probs += [prob]
	if len(y) == 0:
	...
	return pred_labels, (ACC, MSE, SCC), pred_values, pred_probs
